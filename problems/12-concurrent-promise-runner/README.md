# Concurrent promise runner

## Problem

Run a list of async jobs with a maximum concurrency limit while preserving input order.

## Signature

```ts
runWithLimit<T>(tasks: Array<() => Promise<T>>, concurrency: number): Promise<T[]>
```
