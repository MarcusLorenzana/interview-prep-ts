...
Implement a rate limiter that allows at most 5 requests per user in any rolling 60-second window.

## Signature

```ts
class RateLimiter { allow(userId: string, timestamp: number): boolean }
```
